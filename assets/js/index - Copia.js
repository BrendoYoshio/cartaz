jQuery(document).ready(function() {
  ...
  function desenhar() {
    ...
    validador("#descricao", canvas.width/2, 355*scale, "Descrição descrição");
    ctx.font = 25*scale+`px "${FONT_NAME_2}"`;
    validador("#marca", canvas.width/2, 423*scale, "Marca");
    ctx.font = 80*scale+`px "${FONT_NAME_1}"`;
    ...
    validador("#preco_por", canvas.width/2, 667*scale, "00");
    if (widthPrecoPor==0) {
      widthPrecoPor = ctx.measureText("00").width;
    }
    ctx.font = 15*scale+`px "${FONT_NAME_2}"`;
    validador("#ean13", canvas.width/2, 717*scale, "1234567890123");
    if (href == "normal") {
      ctx.font = 50*scale+`px "${FONT_NAME_3}"`;
      ctx.fillText("APENAS", canvas.width/2, 494*scale);
